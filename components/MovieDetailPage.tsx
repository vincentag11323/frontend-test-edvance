...
export default function MovieDetailPage({title, original_title, adult, original_language, popularity, overview, poster_path, backdrop_path, genres} : MovieDetail){
  ...
  useEffect(() => {
    ...
    return  <div className="w-full h-full flex flex-col gap-2 justify-start items-start p-2">
                         <Button onClick={() => router.push('/')}> BACK </Button>
 

      <div className=" w-full h-full flex justify-center p-5">   <div className="w-full sm:max-w-sm md:max-w-md lg:max-w-lg h-full  shadow-lg p-2 flex-col align-middle justify-around rounded-lg hover:shadow-xl transform hover:cursor-pointer">
      <div className=" w-[80%] mx-auto flex justify-center mb-1 my-auto min-h-[300px]">   {imgSrc ? (
                   <Image
                     src={imgSrc}
                     width={200}
                     preload
        ...
        <div className="mx-auto mb-5 flex justify-center gap-5 ">
            ...
            className=""
          > */}
            {genres.map(v => v.name).map(v => (<Chip
            label={v}
            size="small"
          />)
          )}
